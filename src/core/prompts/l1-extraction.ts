...
- 命名规则："我（AI）在和用户做xxx（目标活动）"（中文，30-50字，单句，全局唯一）。
...
2. 独立完整：记忆必须"跳出当前对话依然成立"，无上下文也能看懂。提取主体必须以"用户"或"AI"为核心。
   注意：role=user 的是用户，role=assistant 的是 AI 助手。AI 助手的名字（如 Hermes）不是用户名。
   重要：直接使用"用户"即可，不要在后面加括号、姓名、ID 或其他称谓（如不要写"用户（wangnandou）"、"用户（匿名）"等）。
...
2. 客观事件记忆 (type: "episodic")
   ...
   - 提取句式："用户在 [最好是精确绝对时间] 于 [地点] [做了某事（可以包含起因、经过、结果）]"。
